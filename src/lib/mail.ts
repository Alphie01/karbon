...
const MAIL_HOST = process.env.MAIL_HOST || "mail.example.com";
const MAIL_USER = process.env.MAIL_USER || "info";
const MAIL_PASS = process.env.MAIL_PASS || "";

// 1. SMTP Transport for Sending
export const sendEmail = async (to: string, subject: string, html: string, fromName = "EcoPilot") => {
    try {
        ...
        const info = await transporter.sendMail({
            from: `"${fromName}" <${process.env.MAIL_FROM || `${MAIL_USER}@${MAIL_HOST}`}>`,
            to,
            subject,
            html
        });
