...
$rc = "cd ${REMOTE_DIR}; tar -xzf source.tar.gz; rm source.tar.gz; npm install --legacy-peer-deps; npx prisma db push; npx prisma generate; npm run build; cp -r public .next/standalone/; cp -r .next/static .next/standalone/.next/; pm2 restart blt-platform"
